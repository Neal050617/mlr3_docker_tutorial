...
setwd("/Users/colinliu/Desktop/mlr3_docker_tutorial/data_for_analysis/20250211-mlr-JINYU/20250808")
...
######## Function ##################
# 修改后的统计检验函数
perform_statistical_test <- function(tbl_result) { # tbl_result <- tbl
    ttbl <- tbl_result$table
    feature_name <- tbl_result$feature_name

    # 确保至少有2x2表格
    if (nrow(ttbl) >= 2 && ncol(ttbl) >= 2) {
        tryCatch(
            {
                # 计算期望频数
                expected <- chisq.test(ttbl)$expected

                # 判断使用哪种检验
                if (any(expected < 5)) {
                    # 使用费歇尔精确检验
                    test_result <- fisher.test(ttbl, simulate.p.value = TRUE)
                    method <- "Fisher's Exact Test"
                } else {
                    # 使用卡方检验
                    test_result <- chisq.test(ttbl)
...
prepare_feature_data <- function(data = data14, 
                                focus_on = "feature13",
                                focus_on2 = NULL,
                                feature_colnames = tNGS_colnames$new_name,
                                total_samples = nrow(data)) {
  # 构建分组变量
  group_vars <- c(focus_on, focus_on2)

  data |>
    select(all_of(group_vars), feature_colnames) |>
    ...
    pull(data2) |> 
    reduce(full_join) |>  # 全连接合并, by = "features"
...
create_feature_plot <- function(plot_data=Data, sig_features = significant_features, 
    ...
    plot_data_long <- plot_data |>
        ...
        mutate(
            level = factor(str_remove(level, "_per") |> str_remove("level_")) |>
                fct_recode(!!color_labels[1] := "0",!!color_labels[2] := "1"),
    ...
    w <- ggplot(plot_data_long, aes(x = old_name, y = percentage, fill = level)) +
        ...
        geom_text(
            aes(label = display_label, y = percentage * 1.005),
...
# 数据预处理函数
prepare_feature_data3 <- function(data = data14, 
                                 focus_on = "feature09",
                                 focus_on2 = "feature10",
                                 feature_colnames = "feature13",
                                 total_samples = nrow(data)) {
  # 构建分组变量
  group_vars <- c(focus_on, focus_on2)
  
  data |>
    select(all_of(c(group_vars,feature_colnames))) |>
    rownames_to_column("id") |>
    as_tibble() |>
    pivot_longer(
      cols = -c(id, all_of(c(group_vars))),
      names_to = "features",
      values_to = "value"
    ) |>
    # 按feature13和features分组计算
...
Data <- prepare_feature_data(
    ...
    focus_on2 = NULL, # 分析特征list_data2[[1]]
    feature_colnames = tNGS_colnames$new_name  # 明确传递列名信息
)
...
Data <- prepare_feature_data(
    ...
    feature_colnames = feature_colnames$new_name  # 明确传递列名信息
)
sLevels <- sort(unique(na.omit(data14[[list_data2[[2]]]])))
sData <- tibble(!!sym(list_data2[[2]]) := sLevels) %>%
  mutate(data = map(!!sym(list_data2[[2]]), ~ prepare_feature_data(
    data = data14 %>% filter(!!sym(list_data2[[2]]) == .x),
    focus_on = focus_on,
    feature_colnames = feature_colnames$new_name
  )))
c_list <- c("1"="男","0"="女")
lapply(1:nrow(sData), function(i) {# i <- 1
    ...
    PP <- create_feature_plot(plot_data = sData$data[[i]] %>% 
                               arrange(desc(rowSums(across(starts_with("level_"))))) %>% head(10), 
             sig_features = significant_features, 
...
focus_on <- "feature13"
data14 <- data14 %>%
  mutate(feature10 = case_when(
    feature10 < 3 ~ 1,
    feature10 >= 3 & feature10 < 6 ~ 2,
    feature10 >= 6 & feature10 <= 14 ~ 3,
    TRUE ~ NA_real_
  ))
...
sData <- Data %>% group_by(!!sym(list_data2[[analysis_id]])) %>% nest() %>% filter(!is.na(feature10))
...
sLevels <- sort(unique(na.omit(data14[[list_data2[[analysis_id]]]])))
sData <- tibble(!!sym(list_data2[[analysis_id]]) := sLevels) %>%
  mutate(data = map(!!sym(list_data2[[analysis_id]]), ~ prepare_feature_data(
    data = data14 %>% filter(!!sym(list_data2[[analysis_id]]) == .x),
    focus_on = focus_on,
    feature_colnames = feature_colnames$new_name
  )))
...
lapply(1:nrow(sData), function(i) {# i <- 4
    # 使用修改后的函数
    significant_features <- map_df(feature_colnames$new_name, ~ {
        tbl <- create_contingency_table(focus_on, .x,
            focus_on2 = list_data2[[analysis_id]],
    ...
    # 检查significant_features是否为空
    if (is.null(significant_features) || nrow(significant_features) == 0 || 
        all(is.na(significant_features))) {
      
      # 记录警告信息
      warning_msg <- paste0(
        "第", i, "行 (", 
        c_list[as.character(sData[[list_data2[[analysis_id]]]][i])], 
        ") significant_features为空，跳过此数据\n"
      )
      cat(paste0(Sys.time(), " - 警告: ", warning_msg), 
...
lapply(1:nrow(sData), function(i) { # i <- 2
    ...
    PP <- create_feature_plot(
        plot_data = sData$data[[i]] %>% arrange(desc(rowSums(across(starts_with("level_"))))) %>% head(10),
        #arrange(desc(total_per)) %>% head(10),
        sig_features = significant_features,
        title = paste0(c_list[as.character(sData[[list_data2[[analysis_id]]]][i])], "普通和重症肺炎病原体分布统计图"),
        x_label = "病原体",
...
lapply(1:nrow(sData), function(i) { # i <- 1
    ...
    PP <- create_feature_plot2(
        plot_data = sData$data[[i]] %>%
            arrange(desc(total_per)) %>% head(10),
        sig_features = significant_features,
        title = paste0(F_list[as.character(sData[[focus_on]][i])], "肺炎病原体分布统计图"),
        x_label = "病原体",
        y_label = "检出率(%)",
        color_labels = c_list
    )
...
Data <- prepare_feature_data2(
    data = data14 %>% filter(!is.na(feature65)),
    focus_on = focus_on, # 分析特征
    focus_on2 = list_data2[[analysis_id]], # 分析特征
    feature_colnames = feature_colnames$new_name# 明确传递列名信息
)
...
# 14.不同年龄、不同性别的混合感染，重症、非重症情况分析 ########################

analysis_id <- 14
feature_colnames <- "feature10"
focus_on <- "feature13"

Data <- prepare_feature_data3(
  data = data14,
  focus_on =  "feature09", # 分析特征
  focus_on2 = "feature10", # 分析特征
  feature_colnames = "feature13"   # 明确传递列名信息
) |> filter(!is.na(feature10))

F_list <- c("1"="幼儿组","2"="学龄前组","3"="学龄组")
c_list <- c("0" = "普通肺炎", "1" = "重症肺炎")
sData <- Data %>% group_by(feature10) %>% nest()

sig_features <- lapply(1:nrow(sData), function(i) {# i <- 1
  # 使用修改后的函数
  tbl <- create_contingency_table(focus_on = "feature09", feature_name = "feature13",
                                  focus_on2 = "feature10", 
                                  focus_on2_subset = as.numeric(as.character(sData[["feature10"]][i]))
  )
  significant_features <- perform_statistical_test(tbl)
  return(significant_features)
}) %>% bind_rows()

plot_data <- Data
# 合并显著性标记
color_labels <- c("0"="女","1"="男")
plot_sign <- plot_data |>
  bind_cols(sig_features %>% select(-feature)) |>
  mutate(feature = map_chr(feature10,~F_list[.x]),
         feature = factor(feature,levels=F_list)) |>
  arrange(feature10)
# 转换绘图数据格式  
plot_data_long <- plot_data |>
  pivot_longer(
  ...
  mutate(
    level2 = factor(str_remove(level, "_per") |> str_remove("level_")) |>
      fct_recode(!!color_labels[1] := "0",!!color_labels[2] := "1"),
    count_label = ifelse(level == color_labels[1], level_0, level_1),
    display_label = paste0(count_label, "\n(", round(percentage,2), "%)")
  ) |>
  mutate(feature = map_chr(feature10,~F_list[.x]),
         feature = factor(feature,levels=F_list)) |>
  mutate(level = map_chr(level,~str_replace_all(.x,"_per",""))) |>
  left_join(plot_data |>
              pivot_longer(
                cols = c(level_0, level_1),
                names_to = "level",
                values_to = "value"
              ) |> select(feature10,level,value)
  ) |>
  arrange(feature10)

w <- ggplot(plot_data_long, aes(x = feature, y = percentage, fill = level2)) +
  geom_col(position = position_dodge(0.8), width = 0.7) +
  scale_fill_manual(values = setNames(c("#D99090","#4F81BD"), color_labels)) +
  geom_text(
    aes(label = display_label, y = percentage * 1.005),
    position = position_dodge(width = 0.8),
    size = 3,
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15)))
showtext_auto() # 自动启用中文支持
ggsave(paste0("./14.不同年龄段-不同性别-重症率.pdf"), w, width = 10, height = 8)

addWorksheet(wb, "14.sign")
writeData(wb, "14.sign", plot_sign)
addWorksheet(wb, "14.data")
writeData(wb, "14.data", plot_data_long)
