...
# 设置并行计算 - 减少工作进程数以避免资源竞争
future::plan("multisession", workers = 3) # 使用3个并行工作进程
set.seed(123)
mlr3verse_info()
...
data <- data14 |>
  ...
  filter(feature14_2 == 1) |> dplyr::select(!starts_with("feature_14_"))
...
cat("预处理后数据维度:", dim(task_prep$data()), "\n")
cat("目标变量分布:\n")
print(table(task_prep$data()[[task_prep$target_names]]))
cat("特征数量:", length(task_prep$feature_names), "\n")
...
model_params <- list(
  ...
  rf = ps(
    num.trees = p_int(lower = 100, upper = 500),  # 减少树的数量范围
    min.node.size = p_int(lower = 5, upper = 20),  # 增加最小节点大小
    mtry = p_int(1, 15),  # 限制mtry范围
    sample.fraction = p_dbl(0.6, 0.9)  # 添加抽样比例参数
  ),
  svm = ps(
...
learner_base_rf <- lrn("classif.ranger",
  ...
  num.trees = 500,
  importance = "impurity",
  min.node.size = 5,  # 增加最小节点大小
  replace = TRUE,     # 使用有放回抽样
  sample.fraction = 0.8,  # 抽样比例
  respect.unordered.factors = "order"  # 处理因子变量
)
...
## 3.6. 执行基准测试
cat("开始执行基准测试...\n")
cat("训练集维度:", dim(task_train_selected$data()), "\n")
cat("特征数量:", length(task_train_selected$feature_names), "\n")
cat("目标变量分布:\n")
print(table(task_train_selected$data()[[task_train_selected$target_names]]))
...
tryCatch({
  ...
  cat("基准测试出错:", e$message, "\n")
  # 尝试单独测试每个学习器
  cat("尝试单独测试每个学习器...\n")
  for(learner_name in names(auto_tuners)) {
    cat("测试学习器:", learner_name, "\n")
    tryCatch({
      single_grid <- benchmark_grid(
        tasks = task_train_selected,
        learners = auto_tuners[learner_name],
        resamplings = rsmp("holdout", ratio = 0.8)  # 使用更简单的重抽样
      )
      single_bmr <- benchmark(single_grid, store_models = TRUE)
      cat("学习器", learner_name, "测试成功\n")
    }, error = function(e2) {
      cat("学习器", learner_name, "测试失败:", e2$message, "\n")
    })
  }
  stop(e)
})
