...
# 添加寻找拐点的函数（在mytheme函数之后添加）
find_elbow_point <- function(importance_values) {
  # 对重要性值进行排序
  sorted_vals <- sort(importance_values, decreasing = TRUE)
  ...
  for(i in 1:n) {
    distances[i] <- abs(
      (line_end$y - line_start$y) * points$x[i] - 
      (line_end$x - line_start$x) * points$y[i] + 
...
# 修改自动调优器的定义（在定义auto_tuner之前）
feature_selection_wrapper <- function(learner) {
  GraphLearner$new(
    po("colapply", id = "importance_filter",
       applicator = function(x) {
         # 计算特征重要性
         coef_matrix <- coef(x$model, s = x$lambda)
         coeffs <- as.vector(coef_matrix)[-1] # 移除截距
         importance <- abs(coeffs) / max(abs(coeffs))
         
         # 找到拐点
         cutoff_idx <- find_elbow_point(importance)
         threshold <- sort(importance, decreasing = TRUE)[cutoff_idx]
...
at = auto_tuner(
  ...
  learner = as_learner(
    pbp_prep %>>% 
    feature_selection_wrapper(learner_base_glmnet)
  ),
...
# 在保存结果之前添加特征重要性可视化
# 在生成model_report.xlsx之前添加
# 创建特征重要性的可视化
importance_plot <- ggplot(feature_table, 
  aes(x = reorder(特征名称, abs(系数值)), y = abs(系数值))) +
...
# 添加拐点分析结果
elbow_idx <- find_elbow_point(abs(feature_table$系数值))
cat("\n拐点位置:", elbow_idx)
cat("\n建议保留的特征数量:", elbow_idx)
cat("\n对应的重要性阈值:", feature_table$重要性[elbow_idx])
