...
if (TRUE) {
    option_list <- list(
        ...
        make_option(c("--part"), type = "character", default = "8/11", help = "咋分的样本，2/3还是3/4，或者0.8"),
        make_option(c("--split"), type = "character", default = "split.map-group4.txt", help = "客户指定train和testsplit.map-group.txt"),
        ...
        make_option(c("--n_features"), type = "numeric", default = 5, help = "RFE算法选择的特征数量"),
        ...
        make_option(c("--valid"), type = "character", default = "final.fix.valid.rarefac.ASV_table.xls", help = "排列组合"),
        ...
        make_option(c("-s", "--seed"), type = "numeric", default = "4321", help = "设置种子,默认1234"),
        # 路径设置
        make_option(c("--cores"), type = "double", default = 6, help = "准备给多少线程分析啊兄弟"),
...
# 检查是否已存在efs.RData文件
efs_rdata_path <- file.path(opts$outdir, "efs.RData")
if (file.exists(efs_rdata_path)) {
    cat("发现已存在的efs.RData文件，加载数据并跳转到外部验证部分...\n")
    load(efs_rdata_path)
    cat("数据加载完成，跳转到外部验证部分\n")
    goto_external_validation <- TRUE
} else {
    goto_external_validation <- FALSE
}

# 如果需要跳转到外部验证，则跳过模型训练部分
if (!goto_external_validation) {
    cat("开始完整的建模流程...\n")
...
if (!isTRUE(opts$trainOnly)) {
    ...
    if (opts$split == "none") {
        ...
        split_gp_pre0 <- split_gp_pre %>%
            ...
            mutate(
                train = map(data2, ~ factor(pull(.x, SampleID), levels = Gp$SampleID) %>% as.numeric()),
                test = map(data3, ~ factor(pull(.x, SampleID), levels = Gp$SampleID) %>% as.numeric())
            )
...
if (opts$valid != "none") {
    ...
    cat("外部验证数据维度：", nrow(Valid_Data), "特征 x", ncol(Valid_Data)-1, "样本\n")
    ...
    if (opts$select != "none") {
        ...
        cat("应用特征选择列表，保留", nrow(Valid_Data), "个特征\n")
    }
    if (opts$delete != "none") {
        dd <- read_tsv(opts$delete, col_names = F) %>% rename_at(1, ~"Features")
        Valid_Data <- Valid_Data %>% filter(!Features %in% (dd %>% t() %>% as.character()))
        cat("应用特征删除列表，剩余", nrow(Valid_Data), "个特征\n")
    }
    ...
    if (opts$map2 != "none") {
        ...
        if (opts$gp != "none") {
            ...
            cat("外部验证分组：", paste(gp_valid, collapse = " vs "), "\n")
        } else {
            Valid_Gp <- Valid_Gp %>%
                mutate(group = factor(group, levels = gp))
            cat("外部验证分组：", paste(gp, collapse = " vs "), "\n")
        }
        
        cat("外部验证样本数：", table(Valid_Gp$group), "\n")
        
    } else {
        stop("请提供外部验证分组文件(--map2参数)！")
    }
    
    # 8.3 数据预处理 - 与训练数据保持一致的格式
    ...
    # 使用与训练数据相同的列名映射（必须保持特征名一致）
    common_features <- intersect(names_values$old_colnames[-length(names_values$old_colnames)], 
    ...
    # 对缺失的选中特征进行补零并对齐列顺序
    missing_features <- setdiff(selected_features, colnames(Valid_Data))
    if (length(missing_features) > 0) {
        if (isTRUE(opts$fill_missing)) {
            for (feat in missing_features) {
                Valid_Data[[feat]] <- 0
            }
            cat("已为外部验证数据补全缺失特征（用0）：", paste(missing_features, collapse = ", "), "\n")
        } else {
            cat("未启用补零(--fill_missing=FALSE)。将仅使用外部验证中存在的选中特征。缺失特征：", paste(missing_features, collapse = ", "), "\n")
        }
    }

    # 对齐列顺序
    if (isTRUE(opts$fill_missing)) {
        Valid_Data <- Valid_Data[, c(selected_features, "group")]  # 若补零，则按完整选中特征顺序对齐
    } else {
        available_features_raw <- intersect(selected_features, colnames(Valid_Data))
        Valid_Data <- Valid_Data[, c(available_features_raw, "group")]  # 否则仅保留可用特征
    }
    
    # 8.4 创建外部验证任务并应用与训练一致的预处理(使用已训练好的管道进行predict)
    ...
    # 使用训练阶段拟合好的预处理管道对外部验证集进行predict，确保特征不被错误删除
    valid_task_prep <- valid_task # pbp_prep_trained$predict(valid_task)[[1]]
    cat("外部验证预处理后特征数：", length(valid_task_prep$feature_names), "\n")
    
    # 8.5 选择与训练模型相同的特征（按训练阶段的选中特征对齐，若个别在预处理映射后不存在则剔除并提示）
    ...
    for (model_name in names(model_results)) {
        ...
        tryCatch({
            ...
            valid_performance <- valid_pred$score(msrs(c("classif.auc", "classif.acc", "classif.ce")))
            
            # 准备ROC数据
            valid_roc_data <- data.frame(
                truth = as.integer(valid_pred$truth == valid_task_selected$positive),
                prob = valid_pred$prob[, valid_task_selected$positive],
            ...
            valid_roc_all[[model_name]] <- valid_roc_data %>% mutate(model = model_name)
            
            cat("  - AUC:", round(valid_performance["classif.auc"], 4), 
    ...
    for (i in 1:length(names(valid_roc_all))) {
        ...
        # 外部验证ROC和箱线图
        p_valid <- roc_box_plot(valid_roc_all[[model_name]], title = "External Validation", label = model_name)
        ...
        if (exists("Plot_data4") && model_name %in% names(Plot_data4)) {
            ...
            p4_valid <- ggplot() + theme_void() + labs(title = "特征重要性不可用")
        }
        
        # 生成外部验证综合图
        ComBn_plot(list(p_valid[1:2]), 
            w = p4_valid, out_parm = opts$outdir, 
            biomaker_num_fix_parm = paste0(model_name, "_external_validation")
        )
    }
    
    # 8.10 更新Excel报告，包含外部验证结果
    cat("\n更新Excel报告...\n")
    
    # 添加外部验证性能比较sheet
    addWorksheet(wb, "外部验证比较")
    writeData(wb, "外部验证比较", performance_comparison_with_valid, startRow = 1)
    setColWidths(wb, "外部验证比较", cols = 1:ncol(performance_comparison_with_valid), widths = "auto")
    ...
    addStyle(wb, "外部验证比较", headerStyle, rows = 1, cols = 1:ncol(performance_comparison_with_valid), gridExpand = TRUE)
    ...
    addStyle(wb, "外部验证比较", tableStyle,
        rows = 1:(nrow(performance_comparison_with_valid) + 1),
        cols = 1:ncol(performance_comparison_with_valid), gridExpand = TRUE
    )
    
    # 为外部验证列设置颜色阶
    valid_cols <- grep("Valid_", colnames(performance_comparison_with_valid))
    for (col in valid_cols) {
        conditionalFormatting(wb, "外部验证比较",
            cols = col,
            rows = 2:(nrow(performance_comparison_with_valid) + 1),
            type = "colorScale",
            style = c("#E41A1C", "#FFFFFF", "#4F81BD")
        )
    }
    
    # 在每个模型的sheet中添加外部验证结果
    for (model_name in names(valid_results)) {
        if (model_name %in% names(wb)) {
            # 获取当前sheet的行数
            current_data <- readWorkbook(wb, sheet = model_name)
            if (is.null(current_data)) current_data <- data.frame()
            start_row <- nrow(current_data) + 3
            
            # 添加外部验证性能
            writeData(wb, model_name, "外部验证性能", startRow = start_row, startCol = 1)
            
            valid_perf_data <- data.frame(
                Metric = c("AUC", "Accuracy", "Classification Error"),
                External_Validation = c(
                    valid_results[[model_name]]$performance["classif.auc"],
                    valid_results[[model_name]]$performance["classif.acc"],
                    valid_results[[model_name]]$performance["classif.ce"]
                )
            )
    ...
    cat("\n=== 外部验证分析完成！ ===\n")
    cat("生成的外部验证文件：\n")
    cat("- 外部验证ROC曲线：", file.path(opts$outdir, "valid_roc_plot.pdf"), "\n")
    cat("- 各模型外部验证详细图表：step4.[model_name]_external_validation.patchwork.pdf\n")
    cat("- 外部验证统计检验结果：step4.[model_name]_external_validation_wilcox_test.tsv\n")
    cat("- 更新的Excel报告已包含外部验证结果\n")
    ...
    cat("未提供外部验证数据(--valid参数)，跳过外部验证步骤\n")
}
# 最终保存（包含外部验证结果）
cat("保存完整分析结果（包含外部验证）...\n")
save.image(file.path(opts$outdir, "efs.RData"))
cat("\n所有分析结果已保存到：", file.path(opts$outdir, "efs.RData"), "\n")
